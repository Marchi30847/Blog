...
public interface SpringDataArticleRepository extends CrudRepository<Article, Long> {
    ...
    @Query("SELECT a FROM Article a JOIN User u ON a.author = u WHERE u.email = :authorEmail")
    List<Article> findByAuthor(String authorEmail);

    @Query("SELECT a FROM Article a JOIN Blog b ON a.blog = b WHERE b.name = :blogName")
